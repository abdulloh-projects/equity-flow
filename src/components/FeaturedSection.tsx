...
export default function FeaturedSection() {
  const getBadgeConfig = (badge: string) => {
    switch (badge) {
      case 'hot':
        return { icon: Flame, label: 'Hot', color: 'from-[#B38B2D] to-[#F59E0B]' }; // Warning color + Orange
      case 'closing':
        return { icon: Clock, label: 'Almost Funded', color: 'from-[#274060] to-[#3A5A7A]' };
      case 'new':
        return { icon: Sparkles, label: 'New', color: 'from-[#3A5A7A] to-[#6C7A89]' };
      default:
        return { icon: Sparkles, label: 'Featured', color: 'from-[#274060] to-[#3A5A7A]' };
    }
  };
