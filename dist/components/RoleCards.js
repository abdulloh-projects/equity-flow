      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {roles.map((role, index) => {
            ...
            return (<div key={index} className={`bg-white rounded-2xl p-8 border-2 border-[#DCE3E8] ${role.hoverBorder} transition-all duration-300 hover:shadow-xl group`}>
                ...
                <a href="#" className={`inline-flex items-center space-x-2 ${role.linkColor} hover:opacity-80 transition-opacity duration-200 group`}>
                  <span className="font-semibold">Learn more</span>
                  <lucide_react_1.ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform duration-200"/>
                </a>
              </div>);
        })}
        </div>
